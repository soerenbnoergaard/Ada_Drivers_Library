...
package SAM_SVD.DACC is
   ...
   type MR_SLEEP_Field is
     (
      --  Normal Mode: The DAC Core and reference voltage circuitry are kept ON
      --  between conversions.
      Disabled,
      --  Sleep Mode: The DAC Core and/or reference voltage circuitry are OFF
      --  between conversions.
      Enabled)
     with Size => 1;
   ...
   --  Fast Wake up Mode
   type MR_FASTWKUP_Field is
     (
      --  Normal Sleep Mode: The sleep mode is defined by the SLEEP bit.
      --  Voltage reference is OFF between conversions.
      Stamode,
      --  Fast Wake up after Sleep Mode: the voltage reference is kept ON
      --  between conversions but DAC Core is OFF
      Fastwakeup)
     with Size => 1;
   for MR_FASTWKUP_Field use
     (Stamode => 0,
      Fastwakeup => 1);
   ...
   type DACC_MR_Register is record
      ...
      --  Sleep Mode
      SLEEP          : MR_SLEEP_Field := SAM_SVD.DACC.Disabled;
      --  Fast Wake up Mode
      FASTWKUP       : MR_FASTWKUP_Field := SAM_SVD.DACC.Stamode;
      --  unspecified
