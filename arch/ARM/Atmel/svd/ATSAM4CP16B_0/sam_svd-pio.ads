...
package SAM_SVD.PIO is
   ...
   type DRIVER1_LINE0_Field is
     (
      ...
      Lo_Drive)
     with Size => 2;
   for DRIVER1_LINE0_Field use
     (Hi_Drive => 0,
      Me_Drive => 1,
      Lo_Drive => 2);

   --  PIOA_DRIVER1_LINE array
   type PIOA_DRIVER1_LINE_Field_Array is array (0 .. 15)
     of DRIVER1_LINE0_Field
     with Component_Size => 2, Size => 32;

   --  I/O Drive Register 1
   type PIOA_DRIVER1_Register
     (As_Array : Boolean := False)
   is record
      case As_Array is
         when False =>
         ...
         when True =>
            --  LINE as an array
            Arr : PIOA_DRIVER1_LINE_Field_Array;
      end case;
   end record
     with Unchecked_Union, Size => 32, Volatile_Full_Access,
          Bit_Order => System.Low_Order_First;

   for PIOA_DRIVER1_Register use record
      Val at 0 range 0 .. 31;
      Arr at 0 range 0 .. 31;
   end record;

   --  Drive of PIO line 16
   type DRIVER2_LINE16_Field is
     (
      --  High drive
      Hi_Drive,
      --  Medium drive
      ...
      Lo_Drive)
     with Size => 2;
   for DRIVER2_LINE16_Field use
     (Hi_Drive => 0,
      Me_Drive => 1,
      Lo_Drive => 2);

   --  PIOA_DRIVER2_LINE array
   type PIOA_DRIVER2_LINE_Field_Array is array (16 .. 31)
     of DRIVER2_LINE16_Field
     with Component_Size => 2, Size => 32;

   --  I/O Drive Register 2
   type PIOA_DRIVER2_Register
     (As_Array : Boolean := False)
   is record
      case As_Array is
         when False =>
         ...
         when True =>
            --  LINE as an array
            Arr : PIOA_DRIVER2_LINE_Field_Array;
      end case;
   end record
     with Unchecked_Union, Size => 32, Volatile_Full_Access,
          Bit_Order => System.Low_Order_First;

   for PIOA_DRIVER2_Register use record
      Val at 0 range 0 .. 31;
      Arr at 0 range 0 .. 31;
   end record;
   ...
   type PIO_Peripheral is record
      ...
      --  I/O Drive Register 1
      DRIVER1 : aliased PIOA_DRIVER1_Register;
      --  I/O Drive Register 2
      DRIVER2 : aliased PIOA_DRIVER2_Register;
   end record
     with Volatile;
   ...
   for PIO_Peripheral use record
      ...
      DRIVER1 at 16#118# range 0 .. 31;
      DRIVER2 at 16#11C# range 0 .. 31;
   end record;
