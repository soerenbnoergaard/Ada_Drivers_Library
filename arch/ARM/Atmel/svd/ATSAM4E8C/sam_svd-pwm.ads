...
package SAM_SVD.PWM is
   ...
   subtype PWM_CLK_DIVA_Field is HAL.UInt8;
   subtype PWM_CLK_PREA_Field is HAL.UInt4;
   subtype PWM_CLK_DIVB_Field is HAL.UInt8;
   subtype PWM_CLK_PREB_Field is HAL.UInt4;
   ...
   type PWM_FMR_Register is record
      --  Fault Polarity (fault input bit varies from 0 to 7)
      FPOL           : PWM_FMR_FPOL_Field := 16#0#;
      --  Fault Activation Mode (fault input bit varies from 0 to 7)
      FMOD           : PWM_FMR_FMOD_Field := 16#0#;
      --  Fault Filtering (fault input bit varies from 0 to 7)
   ...
   type PWM_FSR_Register is record
      --  Read-only. Fault Input Value (fault input bit varies from 0 to 7)
      FIV            : PWM_FSR_FIV_Field;
      --  Read-only. Fault Status (fault input bit varies from 0 to 7)
      FS             : PWM_FSR_FS_Field;
      --  unspecified
   ...
   type PWM_FCR_Register is record
      --  Write-only. Fault Clear (fault input bit varies from 0 to 7)
   ...
   subtype PWM_WPCR_WPCMD_Field is HAL.UInt2;
   ...
   subtype PWM_WPCR_WPKEY_Field is HAL.UInt24;
   ...
   subtype CAE_ADEDGV_Field is HAL.UInt24;

   --  Channel Additional Edge Mode
   type CAE0_ADEDGM_Field is
     (
      --  The additional edge of the channel x output waveform occurs when
      --  CCNTx reaches ADEDGV and the counter of the channel x is
      --  incrementing.
      Inc,
      --  The additional edge of the channel x output waveform occurs when
      --  CCNTx reaches ADEDGV and the counter of the channel x is
      ...
      Dec,
      --  The additional edge of the channel x output waveform occurs when
      --  CCNTx reaches ADEDGV, whether the counter is incrementing or not.
      Both)
     with Size => 2;
   for CAE0_ADEDGM_Field use
     (Inc => 0,
      Dec => 1,
      Both => 2);

   --  PWM Channel Additional Edge Register (ch_num = 0)
   type CAE_Register is record
      --  Channel Additional Edge Value
      ADEDGV         : CAE_ADEDGV_Field := 16#0#;
      --  Channel Additional Edge Mode
      ADEDGM         : CAE0_ADEDGM_Field := SAM_SVD.PWM.Inc;
      --  unspecified
   ...
   for CAE_Register use record
      ADEDGV         at 0 range 0 .. 23;
      ADEDGM         at 0 range 24 .. 25;
      Reserved_26_31 at 0 range 26 .. 31;
   end record;

   subtype CAEUPD_ADEDGVUP_Field is HAL.UInt24;

   --  Channel Additional Edge Mode Update
   type CAEUPD0_ADEDGMUP_Field is
     (
      --  The additional edge of the channel x output waveform occurs when
      --  CCNTx reaches ADEDGVUP and the counter of the channel x is
      --  incrementing.
      Inc,
      --  The additional edge of the channel x output waveform occurs when
      --  CCNTx reaches ADEDGVUP and the counter of the channel x is
      ...
      Dec,
      --  The additional edge of the channel x output waveform occurs when
      --  CCNTx reaches ADEDGVUP, whether the counter is incrementing or not.
      Both)
     with Size => 2;
   for CAEUPD0_ADEDGMUP_Field use
     (Inc => 0,
      Dec => 1,
      Both => 2);

   --  PWM Channel Additional Edge Update Register (ch_num = 0)
   type CAEUPD_Register is record
      --  Write-only. Channel Additional Edge Value Update
      ADEDGVUP       : CAEUPD_ADEDGVUP_Field := 16#0#;
      --  Write-only. Channel Additional Edge Mode Update
      ADEDGMUP       : CAEUPD0_ADEDGMUP_Field := SAM_SVD.PWM.Inc;
      --  unspecified
      Reserved_26_31 : HAL.UInt6 := 16#0#;
   end record
     with Volatile_Full_Access, Size => 32,
   ...
   for CAEUPD_Register use record
      ADEDGVUP       at 0 range 0 .. 23;
      ADEDGMUP       at 0 range 24 .. 25;
      Reserved_26_31 at 0 range 26 .. 31;
   end record;
   ...
   type PWM_Peripheral is record
      ...
      --  PWM Channel Additional Edge Register (ch_num = 0)
      CAE0     : aliased CAE_Register;
      --  PWM Channel Additional Edge Update Register (ch_num = 0)
      CAEUPD0  : aliased CAEUPD_Register;
      --  PWM Channel Mode Update Register (ch_num = 1)
      CMUPD1   : aliased CMUPD_Register;
      --  PWM Channel Additional Edge Register (ch_num = 1)
      CAE1     : aliased CAE_Register;
      --  PWM Channel Additional Edge Update Register (ch_num = 1)
      CAEUPD1  : aliased CAEUPD_Register;
      --  PWM Channel Mode Update Register (ch_num = 2)
      CMUPD2   : aliased CMUPD_Register;
      --  PWM Channel Additional Edge Register (ch_num = 2)
      CAE2     : aliased CAE_Register;
      --  PWM Channel Additional Edge Update Register (ch_num = 2)
      CAEUPD2  : aliased CAEUPD_Register;
      --  PWM Channel Mode Update Register (ch_num = 3)
      CMUPD3   : aliased CMUPD_Register;
      --  PWM Channel Additional Edge Register (ch_num = 3)
      CAE3     : aliased CAE_Register;
      --  PWM Channel Additional Edge Update Register (ch_num = 3)
      CAEUPD3  : aliased CAEUPD_Register;
   end record
     with Volatile;
   ...
   for PWM_Peripheral use record
      ...
      CAE0     at 16#404# range 0 .. 31;
      CAEUPD0  at 16#408# range 0 .. 31;
      CMUPD1   at 16#420# range 0 .. 31;
      CAE1     at 16#424# range 0 .. 31;
      CAEUPD1  at 16#428# range 0 .. 31;
      CMUPD2   at 16#440# range 0 .. 31;
      CAE2     at 16#444# range 0 .. 31;
      CAEUPD2  at 16#448# range 0 .. 31;
      CMUPD3   at 16#460# range 0 .. 31;
      CAE3     at 16#464# range 0 .. 31;
      CAEUPD3  at 16#468# range 0 .. 31;
   end record;
