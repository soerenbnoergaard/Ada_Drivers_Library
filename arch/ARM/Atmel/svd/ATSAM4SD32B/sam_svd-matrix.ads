...
package SAM_SVD.MATRIX is
   ...
   type MATRIX_MCFG_Register is record
      --  Undefined Length Burst Type
      ULBT          : MATRIX_MCFG_ULBT_Field := SAM_SVD.MATRIX.Infinite;
      --  unspecified
   ...
   --  Arbitration Type
   type MATRIX_SCFG_ARBT_Field is
     (
      ...
      Fixed_Priority)
     with Size => 2;
   for MATRIX_SCFG_ARBT_Field use
     (Round_Robin => 0,
      Fixed_Priority => 1);
   ...
   type MATRIX_SCFG_Register is record
      ...
      --  Arbitration Type
      ARBT           : MATRIX_SCFG_ARBT_Field := SAM_SVD.MATRIX.Round_Robin;
      --  unspecified
