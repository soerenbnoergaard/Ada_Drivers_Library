...
package SAM_SVD.ADC is
   ...
   type CTRLC_DUALSELSelect is
     (
      --  Start event or software trigger will start a conversion on both ADCs
      Both,
      --  START event or software trigger will alternatingly start a conversion
      --  on ADC0 and ADC1
      Interleave)
     with Size => 2;
   for CTRLC_DUALSELSelect use
     (Both => 0,
      Interleave => 1);
   ...
   type ADC_CTRLC_Register is record
      ...
      --  Dual Mode Trigger Selection
      DUALSEL        : CTRLC_DUALSELSelect := SAM_SVD.ADC.Both;
      --  unspecified
   ...
   for ADC_SYNCBUSY_Register use record
      ...
      CTRLC          at 0 range 3 .. 3;
      AVGCTRL        at 0 range 4 .. 4;
      SAMPCTRL       at 0 range 5 .. 5;
      WINLT          at 0 range 6 .. 6;
      WINUT          at 0 range 7 .. 7;
      GAINCORR       at 0 range 8 .. 8;
      OFFSETCORR     at 0 range 9 .. 9;
   ...
   ADC0_Periph : aliased ADC_Peripheral
     with Import, Address => System'To_Address (16#42004400#);
