...
package SAM_SVD.TAL is
   ...
   --  Event Break Request
   type BRKSTATUS_EVBRKSelect is
     (
      --  EVBRK running
      Int_Run)
     with Size => 2;
   for BRKSTATUS_EVBRKSelect use
     (Int_Run => 0);

   --  External Break Request
   type BRKSTATUS_EXTBRKSelect is
     (
      ...
      Ext_Halt)
     with Size => 2;
   for BRKSTATUS_EXTBRKSelect use
     (Int_Run => 0,
      Int_Halt => 1,
      Ext_Run => 2,
      Ext_Halt => 3);
