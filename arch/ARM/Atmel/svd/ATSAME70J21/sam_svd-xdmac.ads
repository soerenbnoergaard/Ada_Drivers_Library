...
package SAM_SVD.XDMAC is
   ...
   type XDMAC_GTYPE_Register is record
      ...
      NB_CH          : XDMAC_GTYPE_NB_CH_Field := 16#0#;
      --  Number of Bytes
      FIFO_SZ        : XDMAC_GTYPE_FIFO_SZ_Field := 16#0#;
      --  Number of Peripheral Requests Minus One
      NB_REQ         : XDMAC_GTYPE_NB_REQ_Field := 16#0#;
      --  unspecified
   ...
   type XDMAC_GCFG_Register is record
      --  Read-only. Configuration Registers Clock Gating Disable
      CGDISREG      : Boolean;
      --  Read-only. Pipeline Clock Gating Disable
      CGDISPIPE     : Boolean;
      --  Read-only. FIFO Clock Gating Disable
      CGDISFIFO     : Boolean;
      --  Read-only. Bus Interface Clock Gating Disable
      CGDISIF       : Boolean;
      --  unspecified
      Reserved_4_7  : HAL.UInt4;
      --  Read-only. Boundary X Kilobyte Enable
      BXKBEN        : Boolean;
      --  unspecified
   ...
   type CIM_Register is record
      --  Write-only. End of Block Interrupt Mask Bit
      BIM           : Boolean := False;
      --  Write-only. End of Linked List Interrupt Mask Bit
      LIM           : Boolean := False;
      --  Write-only. End of Disable Interrupt Mask Bit
      DIM           : Boolean := False;
      --  Write-only. End of Flush Interrupt Mask Bit
      FIM           : Boolean := False;
      --  Write-only. Read Bus Error Interrupt Mask Bit
      RBEIM         : Boolean := False;
      --  Write-only. Write Bus Error Interrupt Mask Bit
      WBEIM         : Boolean := False;
      --  Write-only. Request Overflow Error Interrupt Mask Bit
      ROIM          : Boolean := False;
      --  unspecified
      Reserved_7_31 : HAL.UInt25 := 16#0#;
   end record
   ...
   type CC0_MEMSET_Field is
     (
      ...
      --  This operation is performed on 8, 16 or 32 bits basis.
      Hw_Mode)
     with Size => 1;
   ...
   type CC_Register is record
      ...
      --  Channel Initialization Terminated (this bit is read-only)
      INITD          : CC0_INITD_Field := SAM_SVD.XDMAC.Terminated;
      --  Read in Progress (this bit is read-only)
