...
package SAM_SVD.SYSC is
   ...
   type SUPC_CR_Register is record
      ...
      --  Write-only. This bit must always be written to 0.
      ZERO          : Boolean := False;
   ...
   --  VRVDD Field Selection
   type MR_VDDSEL_Field is
     (
      --  The voltage regulator output value is the factory-programmed value.
      Factory,
      --  The voltage regulator output value is defined by the value programmed
      --  in the field VRVDD.
      User_Vrvdd)
     with Size => 1;
   for MR_VDDSEL_Field use
     (Factory => 0,
      User_Vrvdd => 1);

   --  VRVDD Field Selection
   type MR_VRVDD_Field is
     (
      --  The voltage regulator output value is the factory-programmed value.
      Factory,
      --  The voltage regulator output value is defined by the value programmed
      ...
      User_Vrvdd,
      --  Reset value for the field
      Mr_Vrvdd_Field_Reset)
     with Size => 3;
   for MR_VRVDD_Field use
     (Factory => 0,
      User_Vrvdd => 1,
      Mr_Vrvdd_Field_Reset => 5);
   ...
   --  SRAM1 Power Switch
   type MR_PSWITCH1_Field is
     (
      --  The SRAM1 is not powered.
      Off,
      --  The SRAM1 is powered.
      On)
     with Size => 1;
   for MR_PSWITCH1_Field use
     (Off => 0,
      On => 1);

   --  SRAM2 Power Switch
   type MR_PSWITCH2_Field is
     (
      --  The SRAM2 is not powered.
      Off,
      --  The SRAM2 is powered.
      On)
     with Size => 1;
   for MR_PSWITCH2_Field use
     (Off => 0,
      On => 1);
   ...
   type SUPC_MR_Register is record
      ...
      --  VRVDD Field Selection
      VDDSEL         : MR_VDDSEL_Field := SAM_SVD.SYSC.Factory;
      --  VRVDD Field Selection
      VRVDD          : MR_VRVDD_Field := Mr_Vrvdd_Field_Reset;
      --  POR Core Reset Enable
      ...
      --  SRAM1 Power Switch
      PSWITCH1       : MR_PSWITCH1_Field := SAM_SVD.SYSC.On;
      --  SRAM2 Power Switch
      PSWITCH2       : MR_PSWITCH2_Field := SAM_SVD.SYSC.On;
      --  Password Key
      KEY            : MR_KEY_Field := Mr_Key_Field_Reset;
   end record
     with Volatile_Full_Access, Size => 32,
