...
package SAM_SVD.CRCCU is
   ...
   --  Precomputation Bit Swap Operation of the CRC
   type MR_BITORDER_Field is
     (
      --  CRC computation is performed from the most significant bit to the
      --  least significant bit
      Msbfirst,
      --  CRC computation is performed from the least significant bit to the
      --  most significant bit
      Lsbfirst)
     with Size => 1;
   for MR_BITORDER_Field use
     (Msbfirst => 0,
      Lsbfirst => 1);
   ...
   type CRCCU_MR_Register is record
      ...
      --  Precomputation Bit Swap Operation of the CRC
      BITORDER       : MR_BITORDER_Field := SAM_SVD.CRCCU.Msbfirst;
      --  unspecified
