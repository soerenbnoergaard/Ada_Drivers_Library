...
package SAM_SVD.SYSC is
   ...
   type RTT_MODR_Register is record
      --  Selection of the 32-bit Counter Modulo to generate RTTINC2 flag
      SELINC2        : MODR_SELINC2_Field := SAM_SVD.SYSC.No_Rttinc2;
      --  unspecified
      Reserved_3_7   : HAL.UInt5 := 16#0#;
      --  Selection of the 32-bit Counter Modulo to generate the trigger event
      SELTRGEV       : MODR_SELTRGEV_Field := SAM_SVD.SYSC.No_Event;
      --  unspecified
   ...
   type MR_OSCBYPASS_Field is
     (
      ...
      --  must be set prior to write XTALSEL=1.
      Bypass)
     with Size => 1;
   ...
   type SUPC_MR_Register is record
      ...
      --  unspecified
      Reserved_14_19 : HAL.UInt6 := 16#1#;
      --  Oscillator Bypass
      OSCBYPASS      : MR_OSCBYPASS_Field := SAM_SVD.SYSC.No_Effect;
      --  Cache Data SRAM Power Switch
      CDPSWITCH      : MR_CDPSWITCH_Field := SAM_SVD.SYSC.On;
      --  Cache Tag SRAM Power Switch
      CTPSWITCH      : MR_CTPSWITCH_Field := SAM_SVD.SYSC.On;
   ...
   type WUIR_WKUPT0_Field is
     (
      ...
      --  WKUPDBC on the corresponding wake-up input forces the wake-up of the
      --  core power supply.
      Low,
   ...
   --  WKUP Input Status 0 (cleared on read)
   ...
   type SUPC_SR_Register is record
      ...
      --  Read-only. WKUP Input Status 0 (cleared on read)
   ...
   type SUPC_PWMR_Register is record
      --  Low Power Value Selection
      LPOWERS        : PWMR_LPOWERS_Field := SAM_SVD.SYSC.Factory;
      --  Low Power Value
      ...
      --  Start-up Time when Resuming from Wait Mode
      STUPTIME       : PWMR_STUPTIME_Field := SAM_SVD.SYSC.Fast;
      --  Enhanced Custom Power Value Selection
      ECPWRS         : PWMR_ECPWRS_Field := SAM_SVD.SYSC.Factory;
      ...
      --  SRAM Power Control
      SRAM0ON        : PWMR_SRAM0ON_Field := SAM_SVD.SYSC.Off;
      --  SRAM Power Control
      SRAM1ON        : PWMR_SRAM1ON_Field := SAM_SVD.SYSC.Off;
      --  SRAM Power Control
      SRAM2ON        : PWMR_SRAM2ON_Field := SAM_SVD.SYSC.Off;
      --  SRAM Power Control
      SRAM3ON        : PWMR_SRAM3ON_Field := SAM_SVD.SYSC.Off;
      --  SRAM Power Control
      SRAM4ON        : PWMR_SRAM4ON_Field := SAM_SVD.SYSC.Off;
      --  SRAM Power Control
      SRAM5ON        : PWMR_SRAM5ON_Field := SAM_SVD.SYSC.Off;
      --  SRAM Power Control
      SRAM6ON        : PWMR_SRAM6ON_Field := SAM_SVD.SYSC.Off;
      --  Dual-port RAM Power Control
      DPRAMON        : PWMR_DPRAMON_Field := SAM_SVD.SYSC.Off;
      --  Password Key
