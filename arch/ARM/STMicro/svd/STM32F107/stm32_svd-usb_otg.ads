...
package STM32_SVD.USB_OTG is
   ...
   type USB_OTG_DEVICE_Peripheral is record
      --  OTG_FS device configuration register (OTG_FS_DCFG)
   ...
   for USB_OTG_DEVICE_Peripheral use record
   ...
   --  USB on the go full speed
   USB_OTG_DEVICE_Periph : aliased USB_OTG_DEVICE_Peripheral
     with Import, Address => System'To_Address (16#50000800#);

   type USB_OTG_GLOBAL_Disc is
     (
      Device,
      Host);

   --  USB on the go full speed
   type USB_OTG_GLOBAL_Peripheral
     (Discriminent : USB_OTG_GLOBAL_Disc := Device)
   is record
      --  OTG_FS control and status register (OTG_FS_GOTGCTL)
   ...
   for USB_OTG_GLOBAL_Peripheral use record
   ...
   --  USB on the go full speed
   USB_OTG_GLOBAL_Periph : aliased USB_OTG_GLOBAL_Peripheral
     with Import, Address => System'To_Address (16#50000000#);

   --  USB on the go full speed
   type USB_OTG_HOST_Peripheral is record
      --  OTG_FS host configuration register (OTG_FS_HCFG)
   ...
   for USB_OTG_HOST_Peripheral use record
   ...
   --  USB on the go full speed
   USB_OTG_HOST_Periph : aliased USB_OTG_HOST_Peripheral
     with Import, Address => System'To_Address (16#50000400#);

   --  USB on the go full speed
   type USB_OTG_PWRCLK_Peripheral is record
      --  OTG_FS power and clock gating control register (OTG_FS_PCGCCTL)
      FS_PCGCCTL : aliased FS_PCGCCTL_Register;
   end record
     with Volatile;

   for USB_OTG_PWRCLK_Peripheral use record
      FS_PCGCCTL at 0 range 0 .. 31;
   end record;

   --  USB on the go full speed
   USB_OTG_PWRCLK_Periph : aliased USB_OTG_PWRCLK_Peripheral
     with Import, Address => System'To_Address (16#50000E00#);

end STM32_SVD.USB_OTG;
