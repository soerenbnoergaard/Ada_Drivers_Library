...
package STM32_SVD.ADC is
   ...
   type CR_Register is record
      --  Read-only. ADC enable control
      ADEN          : Boolean := False;
      --  Read-only. ADC disable command
      ADDIS         : Boolean := False;
      --  Read-only. ADC start of regular conversion
      ADSTART       : Boolean := False;
      --  Read-only. ADC start of injected conversion
      JADSTART      : Boolean := False;
      --  Read-only. ADC stop of regular conversion command
      ADSTP         : Boolean := False;
      --  Read-only. ADC stop of injected conversion command
      JADST         : Boolean := False;
      --  unspecified
      Reserved_6_27 : HAL.UInt22 := 16#0#;
      --  ADC voltage regulator enable
      ADVREGEN      : CR_ADVREGEN_Field := 16#2#;
      --  Differential mode for calibration
      ADCALDIF      : Boolean := False;
      --  Read-only. ADC calibration
      ADCAL         : Boolean := False;
   end record
     with Volatile_Full_Access, Size => 32,
          Bit_Order => System.Low_Order_First;
   ...
   subtype CRR_CKMODE_Field is HAL.UInt2;

   --  ADC common control register
   type CRR_Register is record
      --  unspecified
      Reserved_0_15  : HAL.UInt16 := 16#0#;
      --  ADC clock mode
      CKMODE         : CRR_CKMODE_Field := 16#0#;
      --  unspecified
   ...
   subtype DIFSEL_DIFSEL_Field is HAL.UInt18;
